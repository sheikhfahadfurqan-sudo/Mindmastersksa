...
export const Sidebar = ({ activeTab, setActiveTab }) => {
  const { activeRole, isExecutive, canApproveLeave, logout } = useAuthRole();
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: FolderLock,
      badge: activeRole === 'EMPLOYEE' ? '🔒 Restricted' : 'Cleared'
    },
    {
      id: 'hr',
  ...
  return (
    <aside className="w-full lg:w-64 bg-slate-900/60 border-r border-slate-800/80 p-4 flex flex-col justify-between shrink-0">
      <div className="space-y-6">
        ...
        <div className="p-3.5 rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800/80 space-y-2">
          <div className="flex items-center gap-2">
            ...
            {activeRole === 'ADMIN' && <ShieldCheck className="w-4 h-4 text-indigo-400" />}
            {activeRole === 'EMPLOYEE' && <Lock className="w-4 h-4 text-cyan-400" />}
          ...
          <p className="text-[11px] text-slate-400 leading-relaxed">
            ...
            {activeRole === 'ADMIN' && 'Project creation, Task management, Document upload, and Leave approval clearances.'}
            {activeRole === 'EMPLOYEE' && 'Task execution view. Deal values, project budgets, and restricted document downloads are masked.'}
          </p>
      ...
      {/* Footer Info & Sidebar Logout Action */}
      <div className="pt-4 border-t border-slate-800/60 text-[11px] text-slate-500 flex flex-col gap-2">
        <button
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-slate-950 hover:bg-rose-950/80 hover:text-rose-300 border border-slate-800 hover:border-rose-800/80 text-slate-400 text-xs font-semibold rounded-xl transition duration-200 group"
        >
          <LogOut className="w-4 h-4 text-slate-400 group-hover:text-rose-300 transition-colors" />
